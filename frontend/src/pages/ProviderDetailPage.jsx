...
export default function ProviderDetailPage() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 pt-16 pb-14">
      <div className="h-40 sm:h-48 bg-gradient-to-br from-slate-900 via-primary-900 to-primary-700 relative">
        <div className="absolute inset-0 opacity-5" style={{ backgroundImage:'radial-gradient(circle at 2px 2px,white 1px,transparent 0)', backgroundSize:'32px 32px' }} />
      </div>

      <div className="max-w-5xl mx-auto px-4 -mt-20 sm:-mt-24">
        <motion.button initial={{ opacity:0 }} animate={{ opacity:1 }} onClick={() => navigate(-1)}
          className="flex items-center gap-2 text-white/80 hover:text-white mb-4 sm:mb-6 text-sm font-medium group">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 sm:gap-6">
          ...
          <motion.div variants={slideInLeft} initial="hidden" animate="visible" className="lg:col-span-1">
            <Card className="border-0 shadow-xl dark:shadow-gray-900 overflow-hidden lg:sticky lg:top-24">
              <div className="h-1.5 bg-gradient-to-r from-primary-500 to-accent-500" />
              <CardContent className="p-5 sm:p-6 text-center">
                <div className="relative inline-block mb-4">
                  <motion.div whileHover={{ scale:1.05 }}
                    className="w-20 h-20 sm:w-24 sm:h-24 bg-gradient-to-br from-primary-500 to-primary-700 rounded-3xl flex items-center justify-center text-white font-bold text-3xl sm:text-4xl mx-auto shadow-xl overflow-hidden">
                    {avatarSrc
                ...
                <h1 className="font-display text-lg sm:text-xl font-bold text-gray-900 dark:text-white mb-1">{provider.name}</h1>
                ...
                <motion.div whileHover={{ scale:1.02 }}
                  ...
                  <div className="flex items-center justify-center gap-1">
                    ...
                    <span className="text-gray-400 dark:text-gray-500 mx-1 font-bold">–</span>
                    <IndianRupee className="w-4 h-4 text-primary-600 dark:text-primary-400" />
          ...
          {/* Details */}
          <motion.div variants={staggerContainer} initial="hidden" animate="visible" className="lg:col-span-2 space-y-4 sm:space-y-5">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                    {[
                      ...
                      <motion.div key={i} whileHover={{ scale:1.02 }}
                        className="bg-gray-50 dark:bg-gray-700 rounded-xl p-3 sm:p-4 border border-gray-100 dark:border-gray-600 hover:border-primary-200 dark:hover:border-primary-600 transition-colors">
            ...
            {user && role === 'user' && (
              <motion.div variants={fadeUp} custom={3}
                className="bg-gradient-to-r from-primary-600 to-primary-500 rounded-2xl p-5 sm:p-6 text-white flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 shadow-lg shadow-primary-500/25">
                <div>
                ...
                <Button variant="glass" size="lg" asChild className="flex-shrink-0 w-full sm:w-auto">
                  <Link to={`/user/book/${provider._id}`}>Book Now <ChevronRight className="w-4 h-4" /></Link>
                </Button>
              </motion.div>
            )}
