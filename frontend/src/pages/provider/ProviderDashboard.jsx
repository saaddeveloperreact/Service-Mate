...
import {
  fetchProviderBookings, updateBookingStatus, setActiveTab,
...
const STATUS_ACTIONS = {
  pending:       [{label:'Accept',status:'accepted',  Icon:CheckCircle,cls:'bg-blue-500 hover:bg-blue-600 text-white'},{label:'Reject',status:'rejected',Icon:XCircle,cls:'bg-red-500 hover:bg-red-600 text-white'}],
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 pt-16">
          ...
          <motion.div variants={fadeUp} className="flex items-center gap-4 mb-6">
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mt-1 flex-wrap">
                ...
                {user?.rating?.count>0&&<span className="flex items-center gap-1 text-xs text-orange-100"><Star className="w-3 h-3 fill-yellow-200 text-yellow-200"/>{user.rating.average}</span>}
      ...
      <div className="max-w-6xl mx-auto px-4 py-6 sm:py-8">
        <div className="flex items-center justify-between mb-5">
          ...
          {pendingCount>0&&<Badge variant="destructive" className="gap-1.5"><Bell className="w-3 h-3"/>{pendingCount} pending</Badge>}
        </div>
          ...
          <TabsList className="mb-6 flex flex-wrap gap-1 h-auto bg-gray-100 dark:bg-gray-800 p-1 w-full">
            {['all','pending','accepted','in-progress','completed','rejected'].map(tab=>(
              <TabsTrigger key={tab} value={tab} className="capitalize text-xs px-2 sm:px-3 py-1.5 flex-1 sm:flex-none">
                ...
                {tab==='pending'&&pendingCount>0&&<span className="ml-1 bg-red-500 text-white text-xs rounded-full px-1.5 py-0.5 font-bold">{pendingCount}</span>}
              </TabsTrigger>
            ))}
          </TabsList>
                          ...
                          <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
                            <div className="flex items-start gap-3 flex-1 min-w-0">
                              <div className="w-10 h-10 bg-gradient-to-br from-accent-400 to-accent-600 rounded-2xl flex items-center justify-center flex-shrink-0 overflow-hidden">
                                {booking.user?.avatar?<img src={`${API_BASE}${booking.user.avatar}`} alt="" className="w-full h-full object-cover"/>:<User className="w-5 h-5 text-white"/>}
                              </div>
                              <div className="flex-1 min-w-0">
                                ...
                                {booking.address?.city&&<p className="text-xs text-gray-400 dark:text-gray-500 mt-1.5">📍 {[booking.address.street,booking.address.city,booking.address.state].filter(Boolean).join(', ')}</p>}
                                {booking.status==='accepted'&&<span className={`mt-2 inline-flex text-xs font-semibold px-2.5 py-1 rounded-full ${booking.paymentStatus==='paid'?'bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300':'bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400'}`}>{booking.paymentStatus==='paid'?'✅ Payment received':'⏳ Waiting for payment'}</span>}
                              </div>
                            </div>
                            ...
                            <div className="flex flex-row md:flex-col gap-2 flex-shrink-0 flex-wrap">
                                  ...
                                  <div key={idx}>
                                    {action.needsAmount?(
                                      <div className="flex gap-2 items-center">
                                        <div className="relative"><IndianRupee className="absolute left-2.5 top-1/2 -translate-y-1/2 w-3 h-3 text-gray-400"/>
                                          <Input type="number" min="0" placeholder="Total ₹" value={amountMap[booking._id]||''} onChange={e=>setAmountMap({...amountMap,[booking._id]:e.target.value})} className="w-24 text-sm h-9 pl-7"/></div>
                                        <button onClick={()=>handleStatusUpdate(booking._id,action.status,amountMap[booking._id])} className={`flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-semibold ${action.cls}`}><Icon className="w-3.5 h-3.5"/>{action.label}</button>
                                      </div>
                                    ):(
                                      <button onClick={()=>handleStatusUpdate(booking._id,action.status)} className={`flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-semibold ${action.cls}`}><Icon className="w-3.5 h-3.5"/>{action.label}</button>
                                    )}
                                  </div>
                                )
                              })}
                              {booking.status==='completed'&&<button onClick={()=>setReceipt(booking)} className="flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-semibold bg-primary-600 hover:bg-primary-700 text-white shadow-md">🧾 Receipt</button>}
                              {booking.status==='completed'&&booking.totalAmount>0&&(
                                <div className="bg-green-50 dark:bg-green-900/20 rounded-xl p-3 text-center border border-green-200 dark:border-green-800">
                                  <div className="flex items-center justify-center gap-1"><IndianRupee className="w-4 h-4 text-green-600 dark:text-green-400"/><span className="text-xl font-bold text-green-600 dark:text-green-400">{booking.totalAmount}</span></div>
                                  <p className={`text-xs mt-0.5 font-semibold ${booking.paymentStatus==='paid'?'text-green-500':'text-red-400'}`}>{booking.paymentStatus==='paid'?'✅ Paid':'⏳ Pending'}</p>
                                </div>
                              )}
                            </div>
                          </div>
