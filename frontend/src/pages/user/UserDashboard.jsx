...
import { Search, Calendar, Clock, X, IndianRupee, Package, CheckCircle, Activity, QrCode, FileText, CreditCard } from 'lucide-react'
...
export default function UserDashboard() {
  ...
  const handlePaymentDone = async (bookingId) => {
    const res = await dispatch(markPaymentDone(bookingId))
    if (markPaymentDone.fulfilled.match(res)) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 pt-16">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4">
            {stats.map(({label,value,Icon,color},i)=>(
              <motion.div key={i} variants={fadeUp} custom={i}
                ...
                <div className="flex items-center gap-2 mb-2"><Icon className={`w-4 h-4 ${color}`}/><span className="text-xs text-blue-200 font-medium">{label}</span></div>
                  ...
                  {filtered.map((booking,i)=>(
                    <motion.div key={booking._id} initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} exit={{opacity:0}} transition={{delay:i*0.05}}>
                      <Card className="overflow-hidden border-0 shadow-sm hover:shadow-md">
                        <div className={`h-1.5 ${booking.status==='accepted'?'bg-blue-500':booking.status==='completed'?'bg-green-500':booking.status==='in-progress'?'bg-purple-500':booking.status==='pending'?'bg-yellow-400':'bg-gray-300 dark:bg-gray-600'}`}/>
                        <CardContent className="p-4 sm:p-5">
                          ...
                          {booking.description && (
                            <p className="text-sm text-gray-500 dark:text-gray-400 bg-gray-50 dark:bg-gray-700/60 rounded-xl px-3 py-2 border border-gray-100 dark:border-gray-600 mb-3 line-clamp-2">{booking.description}</p>
                          )}
                          <div className="flex flex-wrap items-center gap-2">
                            ...
                            {booking.status==='accepted'&&booking.qrCode&&(
                              booking.paymentStatus==='paid'
                                ? <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300 text-xs font-semibold border border-green-200 dark:border-green-700"><CheckCircle className="w-3.5 h-3.5"/> Payment Done</span>
                                : <Button size="sm" onClick={()=>setQrBooking(booking)} className="h-8 text-xs bg-blue-600 hover:bg-blue-700 gap-1.5"><QrCode className="w-3.5 h-3.5"/> Scan & Pay</Button>
                            )}
                            {booking.status==='accepted'&&!booking.qrCode&&booking.paymentStatus!=='paid'&&(
                              <Button size="sm" onClick={()=>handlePaymentDone(booking._id)} className="h-8 text-xs bg-green-600 hover:bg-green-700 gap-1.5"><CreditCard className="w-3.5 h-3.5"/> Mark Paid</Button>
                            )}
                            {booking.status==='completed'&&(
                              <>
                                <Button size="sm" onClick={()=>setReceipt(booking)} className="h-8 text-xs bg-primary-600 hover:bg-primary-700 text-white gap-1.5"><FileText className="w-3.5 h-3.5"/> View Receipt</Button>
                                <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold ${booking.paymentStatus==='paid'?'bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300 border border-green-200 dark:border-green-700':'bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400 border border-red-200 dark:border-red-700'}`}>
                                  {booking.paymentStatus==='paid'?'✅ Paid':'⏳ Pending'}
                                </span>
                                {booking.paymentStatus!=='paid'&&<Button size="sm" onClick={()=>handlePaymentDone(booking._id)} className="h-8 text-xs bg-green-600 hover:bg-green-700 gap-1.5"><CreditCard className="w-3.5 h-3.5"/> Mark Paid</Button>}
                                {booking.totalAmount>0&&<Badge variant="success" className="font-bold text-sm ml-auto">₹{booking.totalAmount}</Badge>}
                              </>
                            )}
                          </div>
                        </CardContent>
      ...
      {qrBooking&&<QRPaymentModal booking={qrBooking} provider={qrBooking.provider} onClose={()=>setQrBooking(null)} onPaymentDone={()=>handlePaymentDone(qrBooking._id)}/>}
